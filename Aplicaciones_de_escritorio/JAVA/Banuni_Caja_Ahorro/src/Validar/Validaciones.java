...
public class Validaciones{
    ...
    @SuppressWarnings("unused")
    	public boolean Validar_Fechas_hora(String cod) {
        Pattern pat;
        Matcher mat;
        boolean val=false,val1=false;
        pat = Pattern.compile("[0-3][0-9][/][0-1][1-9][/][1-2][0-9][0-9][0-9][ ][0-2][0-4][:][0-5][0-9][:][0-5][0-9]");
		mat = pat.matcher(cod);
		val1= mat.matches();
            if(val1==false)
                return true;
	...
	public boolean Validar_Fechas(String cod) {
		...
		public boolean Validar_code_cliente(String cod) {
	        Pattern pat;
	        Matcher mat;
	        boolean val=false,val1=false;
	        pat = Pattern.compile("[C][0-9][0-9][0-9]");
			mat = pat.matcher(cod);
			val1= mat.matches();
	            if(val1==false)
	                return true;
